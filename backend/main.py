...
def get_technician_calendar_feed(technician_id):
    ...
    for job in jobs:
        ...
        event.end = end_time
        event.description = f"Customer: {customer.name}\nAddress: {customer.address}\nPhone: {customer.phone}\nNotes: {job.notes or 'N/A'}"
        event.location = customer.address
...
def format_job(job):
    ...
    start_datetime = datetime.datetime.combine(job.job_date, job.job_time if job.job_time else datetime.time(9, 0))
    return {
        "id": job.id, "title": f"{customer.name} - {job.description}", "start": start_datetime.isoformat(),
        "description": job.description, "notes": job.notes, "status": job.status,
        "job_date": job.job_date.isoformat(), "job_time": job.job_time.strftime('%H:%M') if job.job_time else None,
        "customer": {"id": customer.id, "name": customer.name, "address": customer.address, "phone": customer.phone, "email": customer.email},
...
def login():
    ...
    if user: return jsonify({"message": "Login Successful", "user": {"email": user.email, "role": user.role}})
...
def dashboard_data():
    ...
    job_status_distribution = [{'name': 'Completed', 'value': completed_jobs}, {'name': 'Scheduled', 'value': scheduled_jobs}]
    recent_activity = [{"id": job.id, "type": "job", "description": f"Job #{job.id} ({job.description}) status: {job.status}"} for job in Job.query.order_by(Job.id.desc()).limit(3).all()]
    dashboard_payload = {"stats": {"totalCustomers": total_customers, "jobsToday": total_jobs, "revenueThisMonth": total_jobs * 150, "inventoryAlerts": 3}, "revenueTrend": [{'name': 'Jan', 'revenue': 13000}, {'name': 'Feb', 'revenue': 15500}, {'name': 'Mar', 'revenue': 18000}, {'name': 'Apr', 'revenue': 15000}, {'name': 'May', 'revenue': 19000}, {'name': 'Jun', 'revenue': 26000}], "jobStatusDistribution": job_status_distribution, "recentActivity": recent_activity}
...
def handle_customers():
    if request.method == 'POST':
        ...
        return jsonify({
            "id": new_customer.id, "name": new_customer.name, "address": new_customer.address, "phone": new_customer.phone, "email": new_customer.email
        }), 201
...
# --- UPGRADED: CUSTOMER DETAIL ENDPOINT ---
@app.route('/api/customers/<int:customer_id>', methods=['GET', 'PUT', 'DELETE'])
def handle_customer(customer_id):
    """Handles GET (details), PUT (update), and DELETE for a single customer."""
    customer = Customer.query.get_or_404(customer_id)

    if request.method == 'GET':
        # Fetch the customer's jobs and format them
        ...
        customer_details = {
            ...
            "jobs": jobs_list # Include the list of jobs
        }
        ...
        db.session.commit()
        return jsonify({
            "id": customer.id, "name": customer.name, "address": customer.address, "phone": customer.phone, "email": customer.email
        })
...
def get_reports_data():
    try:
        ...
        scheduled_jobs = Job.query.filter_by(status='Scheduled').count()
        job_status_distribution = [{'name': 'Completed', 'value': completed_jobs}, {'name': 'Scheduled', 'value': scheduled_jobs}]
        revenue_trend = [{'name': 'Jan', 'revenue': 13000}, {'name': 'Feb', 'revenue': 15500}, {'name': 'Mar', 'revenue': 18000}, {'name': 'Apr', 'revenue': 15000}, {'name': 'May', 'revenue': 19000}, {'name': 'Jun', 'revenue': 26000}]
        jobs_per_technician = db.session.query(User.email, db.func.count(Job.id)).join(Job, User.id == Job.technician_id).group_by(User.email).all()
        technician_performance = [{'name': email.split('@')[0].capitalize(), 'jobs': count} for email, count in jobs_per_technician]
        reports_payload = {"jobStatusDistribution": job_status_distribution, "revenueTrend": revenue_trend, "technicianPerformance": technician_performance}
...
def create_initial_data():
    with app.app_context():
        ...
        db.session.commit()
        job1 = Job(customer_id=customer1.id, technician_id=tech1.id, description='Standard ant treatment', job_date=today, job_time=datetime.time(9, 30), notes="Check under the sink.")
        job2 = Job(customer_id=customer2.id, technician_id=tech2.id, description='Rodent inspection', status='Completed', job_date=today - datetime.timedelta(days=1), job_time=datetime.time(14, 0))
        job3 = Job(customer_id=customer1.id, technician_id=tech1.id, description='Follow-up spider treatment', job_date=today, job_time=datetime.time(11, 0))
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if not os.path.exists(db_path):
            ...
            try:
                Customer.query.with_entities(Customer.email).limit(1).all()
            except Exception:
                print("Customer table appears outdated. Recreating database.")
                create_initial_data()
